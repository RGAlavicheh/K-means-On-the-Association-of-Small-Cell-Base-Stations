...
function [probOFasso,average_num,avg_sum_rate,avg_SCBSs_number,avg_bw_consumption] = func_s3(iter,L,R_B,W,power,X,h,C,cluster)
...
sigma_n2 = -125; % in dB
GAMMA_min = 6;  %in dB
r_SCBS = [20,40,60,80,100]; % requested rate in Mbps
UAVs_number = cluster; 
...
for j = 1:1:UAVs_number
    for i =1:1:length(X)
        ...
        P_LOS = 1/(1+alpha*exp(-beta*((180/pi)*phi-alpha)));
        P_NLOS = 1-P_LOS;
...
for j = 1:1:UAVs_number
    ...
    T_B = 0;
    for kk =1:1:length(A)
        index_of_SE = find(SE_A(:,j) == SE_sort(end-kk+1,j));
        if isempty(index_of_SE)
            continue
        end
        if (T_L < L) && (T_B < W)
            if T_B + w(index_of_SE,j) <= W
                T_L = T_L + 1;
                T_B = T_B + w_A(index_of_SE,j);
            else
                A(index_of_SE,j) = 0;
            end
        else
